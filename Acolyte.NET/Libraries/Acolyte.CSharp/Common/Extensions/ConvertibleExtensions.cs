...
namespace Acolyte.Common
{
    public static class ConvertibleExtensions
    {
        ...
        {
            value.ThrowIfNullValue(nameof(value));
            flag.ThrowIfNullValue(nameof(flag));

            // Get the type code of the input value.
            TypeCode typeCode = value.GetTypeCode();

            // If the underlying type of the flag is signed.
            if (typeCode == TypeCode.SByte || typeCode == TypeCode.Int16 ||
            ...
            throw new NotSupportedException(
                $"The comparison of the type {flag.GetType().Name} is not implemented."
            );
        }
    }
}
